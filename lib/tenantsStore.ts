...
let mockTenants: TenantRecord[] = [
  ...
  },
  {
    ...
    emergency_contact: '+63 917 123 4568',
    status: 'active',
    contract_id: 2,
    unit_id: 102,
  ...
  // Admin 3 (Greenview Residences)
  {
    ...
    emergency_contact: '+63 918 987 6544',
    status: 'active',
    contract_id: 4,
    unit_id: 201,
    unit_number: 'Unit 101',
  ...
  },
  {
    ...
    emergency_contact: '+63 915 111 2223',
    status: 'active',
    contract_id: 5,
    unit_id: 202,
    unit_number: 'Unit 102',
...
export async function getTenantsList(filters?: { query?: string; building_id?: string; admin_id?: number }) {
  await syncDatabase();
  try {
    ...
    if (dbTenants && dbTenants.length > 0) {
      let list = dbTenants.map((t: any) => {
        ...
        const fullName = (firstName || lastName) ? `${firstName} ${lastName}`.trim() : item.User?.email || `Tenant #${item.id}`;
        ...
        return {
          ...
          building_name: latestContract.Unit?.Building?.name || 'Unassigned Property',
...
function applyTenantFilters(records: TenantRecord[], filters?: { query?: string; building_id?: string; admin_id?: number }) {
  let result = [...records];
  if (filters?.admin_id) {
...
export async function createTenant(data: {
  ...
  try {
    ...
    const tenant: any = await Tenant.create({
      admin_id,
      user_id: user.id,
      personal_info_json: { first_name: data.first_name, last_name: data.last_name },
    ...
    // DB Fallback
    const newTenantId = mockTenants.length > 0 ? Math.max(...mockTenants.map(t => t.id)) + 1 : 1;
...
export async function updateTenant(id: number, data: Partial<TenantRecord>, adminId?: number) {
  try {
    ...
    if (tenant) {
      ...
      // Update User email
      if (data.email) {
        await User.update({ email: data.email }, { where: { id: tenant.user_id } });
      }

      // Update Contract
      if (data.unit_id || data.move_in_date || data.move_out_date) {
        const contract: any = await Contract.findOne({ where: { tenant_id: id } });
        if (contract) {
  ...
  const idx = mockTenants.findIndex(t => t.id === id && (!adminId || t.admin_id === adminId));
  if (idx !== -1) {
