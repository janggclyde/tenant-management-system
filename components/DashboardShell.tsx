    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <div className="md:hidden sticky top-0 z-30 flex items-center justify-between bg-white border-b border-gray-200 px-4 py-3 shadow-xs">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-base font-bold text-gray-900 tracking-tight leading-tight block">
              Sylvia
            </span>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto min-h-0">
          {links.map((link) => {
            const isDashboardLink =
              link.name === "Dashboard" ||
              ["/admin", "/super-admin", "/tenant"].includes(link.href);
            ...
            return (
              ...
              >
                <div className="flex items-center">
                  <Icon
                    className={`mr-3 flex-shrink-0 h-5 w-5 ${isActive ? "text-blue-600" : "text-gray-400"}`}
                  />
                  <span>{link.name}</span>
                </div>
                {role === "tenant" &&
      ...
      {/* Main Content */}
      <main
        className={`flex-1 min-w-0 bg-gray-50 ${role === "tenant" ? "pb-24 md:pb-8" : ""}`}
      >
        {children}
      </main>
      ...
      {role === "tenant" && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-200 px-1 py-1.5 flex items-center justify-around shadow-[0_-4px_16px_rgba(0,0,0,0.05)]">
          {links.map((link) => {
            const isDashboardLink =
              link.name === "Dashboard" || link.href === "/tenant";
            const isActive = isDashboardLink
              ? pathname === link.href
              : pathname === link.href || pathname.startsWith(link.href + "/");
            const Icon = link.icon;
            ...
            // Short mobile label
            const label =
              link.name === "Bills & Statements" ||
              link.name === "Bills & Payments"
                ? "Bills"
                : link.name === "Announcements"
                  ? "Notices"
                  : link.name === "Lease & Profile"
                    ? "Profile"
                    : link.name;
            ...
            return (
              ...
              >
                <div className="relative">
                  <Icon
                    className={`w-5 h-5 transition-transform ${isActive ? "scale-110 text-blue-600" : "text-gray-400"}`}
                  />
                ...
                <span
                  className={`text-[10px] mt-1 tracking-tight leading-none ${isActive ? "text-blue-600 font-bold" : "text-gray-500"}`}
                >
                  {label}
                </span>
                {isActive && (
