...
import { updateUnit, deleteUnit } from '@/lib/propertiesStore';
...
export async function PUT(
  ...
  try {
    ...
    const updated = await updateUnit(id, body, adminId);

    if (!updated) {
      return NextResponse.json({ success: false, error: 'Unit not found or access denied' }, { status: 404 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    const deleted = await deleteUnit(id, adminId);
    if (!deleted) {
      return NextResponse.json({ success: false, error: 'Unit not found or access denied' }, { status: 404 });
    }
